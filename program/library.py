...
def viz1(img, interest_points, PATCH_SIZE, threshold, min_sigma, max_sigma, num_sigma):
    """
    ...
    title = 'Blob detection at Threshold=' + str(threshold) + ', min sigma=' + \
...
def viz2(img1, interest_points1, img2, interest_points2, matches, PATCH_SIZE, threshold, min_sigma, max_sigma, num_sigma, from_to):
  ...
  #saving the image locally 
    ...
    ax1.axis('off')
    ax2.axis('off')
    title = 'Matching of ' + str(from_to) + ' Patch Size=' + str(PATCH_SIZE) + ', Threshold=' + str(threshold) + ', min sigma=' + \
    str(min_sigma) + ', max sigma=' + str(max_sigma) + ', num sigma=' + str(num_sigma)
